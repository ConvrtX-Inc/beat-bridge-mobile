...
class queuecustomPlayer extends StatefulWidget {
  List<UserTrackModel> userTrackList = [];
  queuecustomPlayer({required this.userTrackList});
  // var mins;
...
class _customPlayer extends State<queuecustomPlayer> {
  ...
  Future<void> skipNext() async {
    try {
      ...
      await SpotifySdk.pause().then((value) async {
        ...
        var length = (widget.userTrackList.length) - 1;
        if (index <= length) {
          print("my current index= $index");
          await SpotifySdk.play(
                  spotifyUri: widget.userTrackList[index].uri.toString())
              .then((value) {
            buffering();
          });
